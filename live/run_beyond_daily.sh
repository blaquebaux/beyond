#!/bin/bash
# run_beyond_daily.sh — beyond standalone sleeve (Blaque Baux). DRY-RUN by default (logs the target,
# places nothing) using the shared read-only data keys; graduates to PAPER once ~/.config/blaquebaux/alpaca_beyond.env
# exists (that account's own keys). One-time: julia --project=engine -e 'using Pkg; Pkg.instantiate()'.
# Manual dry test:  BB_DRYRUN=1 bash live/run_beyond_daily.sh
set -uo pipefail
REPO="/Users/malcolmx/blaquebaux-beyond"; ENGINE="$REPO/engine"; JULIA="/Users/malcolmx/.juliaup/bin/julia"
DATAENV="$HOME/.config/blaquebaux/alpaca.env"; SLEEVEENV="$HOME/.config/blaquebaux/alpaca_beyond.env"
LOGDIR="$REPO/logs"; mkdir -p "$LOGDIR"; LOG="$LOGDIR/beyond_$(TZ=America/New_York date +%Y%m%d).log"
exec >> "$LOG" 2>&1
echo "======== $(TZ=America/New_York date '+%F %T %Z') beyond daily run ========"
export BB_LEDGER_PATH="$REPO/alpaca_ledger_beyond.sqlite" BB_AUDIT_PATH="$REPO/alpaca_audit_beyond.jsonl"
export BB_HWM_PATH="$HOME/.config/blaquebaux/equity_hwm_beyond.txt" BB_EQUITY_PATH="$HOME/.config/blaquebaux/equity_last_beyond.txt"
...
cd "$REPO" || exit 1
"$JULIA" --project="$ENGINE" "$REPO/live/beyond_live.jl"; RC=$?
